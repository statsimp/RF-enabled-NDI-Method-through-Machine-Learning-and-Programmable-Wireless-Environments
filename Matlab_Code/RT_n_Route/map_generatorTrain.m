...
% map_generatorTrain: is a  function to create 
% the map with ris in configs. This version is used for the training phase
% of an NDI project, where the RIS are configured to create the
% Wavefront Encoding.
% 
% ==============Arguments===================
%
...
if run == 2
    ...
    [paths, maxReflections]= getPathsWF(G, lastConn, Rays, patches_Mat, DoA, label_SDM);
    ...
    % What we do below is the following: we create a cell array where
    % each cell contains another cell array. This inner cell array
    % holds the tile indices used in the ray-routing for the 
    % corresponding wall as its first value. The second value holds 
    % the appropriate normal vector for the corresponding tile.
